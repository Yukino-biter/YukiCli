 ...
 * Agent 循环的退出预算。
 *
 * 把"是否继续下一轮"的主导权交给 LLM 自己——只要它返回 content 不再调用工具，
 ...
 *
 * 配置读取顺序（System.getProperty）：
 * - yukicli.react.token.budget
 * - yukicli.react.stagnation.window
 * - yukicli.react.hard.max.iterations
 */
public class AgentBudget {
    ...
    public static AgentBudget fromSystemProperties() {
        return new AgentBudget(
                readIntProperty("yukicli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("yukicli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("yukicli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
     ...
     * 一旦判定为停滞，状态会保持，后续 check() 会返回 STAGNATION_DETECTED。
     */
    ...
    private static String signatureOf(List<ToolCall> toolCalls) {
        ...
        for (ToolCall tc : toolCalls) {
            sb.append(tc.getName()).append('|').append(tc.getArguments()).append(';');
        }
        return sb.toString();
    }
